...
const db = new Airtable({ apiKey: ACCESS_TOKEN }).base(DB_ID);
const fetchHeaders = {
...
export class AirtableService {
	...
	async fetch({ filters, orderBy } = {}) {
		if (!this.tables.length)
			this.tables = await AirtableService.fetchTables();
	...
	async create(payload) {
		const url = `https://api.airtable.com/v0/${DB_ID}/${this.table}`;
	...
	async update(rowId, payload) {
		const url = `https://api.airtable.com/v0/${DB_ID}/${this.table}/${rowId}`;
		const body = JSON.stringify({
			fields: {
				...payload,
				updated_at: Date.now(),
