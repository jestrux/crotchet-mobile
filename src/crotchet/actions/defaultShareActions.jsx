...
export const addToPinnedItems = {
	...
	match: ({ url, image, file } = {}) =>
		Object.values(cleanObject({ url, image, file })).length,
	handler: async (
		{ image, file, preview, url, title, subtitle, description },
		{ showToast, uploadDataUrl, utils, dbInsert }
	) => {
		image = image || preview;
		...
		try {
			...
			await dbInsert("pinnedItems", payload);
...
export const getShareLink = {
	...
	context: "share",
	match: ({ image, file } = {}) =>
		Object.values(cleanObject({ image, file })).length,
	handler: async (
		{ image, file },
		{ showToast, uploadDataUrl, copyToClipboard, utils }
	) => {
		const asset = file || image;
		try {
			const url = await uploadDataUrl(
				asset.startsWith("http") ? await utils.fetchImage(asset) : asset
			);
			copyToClipboard(url, { message: "Share link copied" });
		} catch (error) {
