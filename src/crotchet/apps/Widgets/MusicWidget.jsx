...
registerAction("previewSpotifyTrack", {
	icon: (
	...
	label: "Preview Spotify Song",
	shortcut: "Shift+Alt+S",
	global: true,
	handler: async (_, { openUrl, toggleAppWindow, showToast }) => {
		try {
...
registerAction("previewSpotifySong", {
	...
	handler: async ({ url }, { openPage }) => {
		if (!url) return;

		const trackId = new URL(url).pathname.replace("/track/", "");
		const track = fetch(`https://api.spotify.com/v1/tracks/${trackId}`, {
			headers: {
		...
		return openPage({
			...
			content: track.then((data) => {
				if (!data?.src) return <div>Track has no preview</div>;
...
const MusicWidget = () => {
	...
	const handleSetMood = (mood, { clear } = {}) => {
		fetchPlaylistDetails(moods[mood], { clear });
		setChangingMood(false);
	};

	const handlePlaySong = (index, { replace } = {}) => {
		if (index === -1) index = 0;
		index === currentSong
			? togglePlay({ stop: replace })
	...
	const fetchPlaylistDetails = async (playlistId, { clear = true } = {}) => {
		if (clear) setPlaylist(null);
		setLoading(true);
			...
			>
				<div className="h-full relative -mr-1">
					<div className="h-full w-full flex flex-col">
						<div className="h-full overflow-auto p-2 flex-1 flex items-start justify-between">
			...
			<Widget
						...
						: [
								{
									...
									handler: () => {
										...
										const mood = moodValues.indexOf(
											playlist.id
										);
										const nextMood =
											Object.keys(moods)[
												mood == moodValues.length - 1
													? 0
													: mood + 1
											];

										console.log("Newxt mood: ", nextMood);

										if (loading)
											return showToast("Loading...");

										handleSetMood(nextMood, {
											clear: false,
										});
									},
								},
				...
				content={
					!playlist
						...
						: {
								video: playlist.poster,
								title: playlist.name,
								subtitle: `by ${playlist.author}, ${playlist.songs.length} songs`,
								onClick: () => handlePlaySong(currentSong),
								button: {
									label: "Open",
									handler: () => openUrl(playlist?.link),
								},
						  }
				}
			>
				{playlist && song && playing && (
					<div className="-mx-3.5 -mb-3.5 mt-1s py-1.5 px-3.5 border-t border-content/10 flex">
						<a
							className="h-full"
							href={playlist?.link}
							target="_blank"
	...
	return (
		<div className="@container h-full">
			<div className="hidden @xs:block h-full">{largeWidget()}</div>
			<div className="@xs:hidden h-full">{smallWidget()}</div>
		</div>
	);
};
